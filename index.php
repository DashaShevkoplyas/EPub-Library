<?php
 ...
 if(isset($_POST['btn'])){
     if(!empty($_POST['select']) && !empty($_POST['name_pub']) && !empty($_POST['url'])){
            ...
            $id_kaf = (int)$select;
            $sql = "INSERT INTO `pubs`(`id_kaf`,`name_pub`, `url`) VALUES ('$id_kaf','$name_pub', '$url')";
...
<body>
    ...
    <main>
    <form  class = "add_pub" action = "/index.php" method="post"> 
         <select id = "select" class ="select" name = "select">
                <option value="1" selected>Кафедра машиновикористання в землеробстві</option>
            ...
            <input id = "name"  type="text" name ="name_pub"class="add-input" autofocus>
            <label class = "url">URL-публікації</label>
            <input id = "url"  type="url" name = "url" class="add-input">  
            
            <input id = "btn" type="submit" value="Додати!" class="add-button" name = "btn">     
  </form>
</main>
    </body>
    </html>